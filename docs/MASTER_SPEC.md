...
This document is the layer underneath it: the concrete architecture, formulas, data shapes and build order that turned that vision into a real, playable, shippable game. Section 12 turns all of it into a single paste-ready prompt for briefing an AI coding agent or a new engineer from scratch.
...
| CI | GitHub Actions: debug APK on every push; a separate, opt-in workflow for a signed release `.aab` | Keep the fast feedback loop (debug build, every push) cheap and separate from the deliberately rare, secret-touching release path. |
...
1. **Background composite** (screen space, outside the camera transform): pre-baked sky → drifting cloud strip → distant skyline (slowest parallax) → warm haze band → a nearer building layer → cool street-level fog. Each strip tiles horizontally and scrolls at its own fraction of camera velocity — the parallax ratios *are* the depth cue.
2. **World layer** (inside the camera transform): facades → power cables → ground strip → props → hazards (explosive barrels) → the persistent decal surface (blood pools, bullet holes, scorch marks — painted once, never redrawn) → corpses → live enemies → player → particles → tracers/slash trails → additive-blended particles on top.
3. **Foreground silhouette layer**: a near plane that scrolls *faster* than the player layer (parallax ratio > 1), anchored strictly below the ground line so it can never occlude a character or a hitbox. This is the one direction of parallax easiest to forget — without something in front of the subject, the eye has no differential-motion cue in that direction at all.
...
5. **Color grade**: warm highlight push + cool shadow lift (two-pass on HIGH/ULTRA; folded into one flat tint on LOW/MEDIUM, where the expensive `overlay`/`soft-light` blend modes are the actual cost) + vignette + animated film grain.
...
- **Hitscan weapons** raycast (slab method, segment vs AABB) against three things in one pass: world colliders (tagged with a `mat` — metal/wood/concrete/sand — that selects the impact particle recipe), explosive barrels, and character hitboxes (a head zone scores a large damage multiplier).
- **Movement is asymmetric, not one blended constant.** Pressing into a direction, releasing to a stop, and reversing hard at speed are three different physical events and get three different acceleration numbers:
  ...
  ```js
  const RUN = 300, SPRINT = 450, JUMP = -900, ACCEL = 2800;
  const STAND_H = 126, CROUCH_H = 63;      // collider height, standing vs crouched
...
- Draw order interleaves body layers and weapon layers (far arm under the gun, near arm over it) so the silhouette reads correctly from either facing direction.
...
- **Aim direction on touch** reads from the operator's own facing/position rather than requiring a second precision-aim gesture layered over movement — touch input real estate is scarce and finger precision is coarse; design the aim model around that constraint rather than porting a mouse-aim assumption unmodified.
- Control scheme is stable across the whole project: Space = jump, 1–4 = weapon slots, WASD/left-stick = move. Never repurpose an already-meaningful key for an unrelated feature (a debug overlay, for instance, belongs on a key nothing else uses — F3, by convention).
...
x += v * dt * m
```
…where `ω = sqrt(k·m)` and `ζ = c/(2ω)`. The failure mode to actively avoid: an under-damped spring (`ζ` well under ~0.6) rings — it overshoots, swings back past zero, and the *next* shot lands on a spring still oscillating from the previous one, which at automatic-weapon fire rates compounds into a visible, un-aimable bounce that reads as "not like a real gun." This is not a subjective judgment call: instrument the muzzle position across a sustained burst, measure peak-to-peak swing and settle time, and tune `k`/`c`/`m` until the *first* excursion (the kick the player is meant to feel) is preserved but the ringing after it is gone. A properly critically-damped setup: one clean kick, ~130ms settle, negligible counter-swing.
...
1. Every install derives a stable **invite code** from a hash of its own random install ID (Crockford-style alphabet — no `I/L/O/U` — so a code read off one phone and typed into another never gets lost to 1/I or 0/O confusion, and a checksum character catches single-typo and adjacent-transposition errors). A friend enters it and is paid a joining bonus immediately.
...
- **`suspicious`** is the buffer state that keeps a flicker of vision (a half-second peek around a corner) from immediately escalating to full alert — awareness has to sustain, not merely spike.
...
- **`combat`**: aim with settling error (wide at first contact, tightening the longer the target stays engaged, and tighter still at higher difficulty), fire in 3–5 round bursts with gaps rather than one continuous stream, track a real magazine with animated reloads, flinch on being hit (rate-limited — a hostile that flinches on *every* connecting round of an automatic weapon never gets to fire back once, which is not difficulty, it is deleting the firefight), retreat when badly hurt.
...
- **Squad awareness**: a hostile going down in view (or earshot) of allies raises their awareness directly — a body is one of the strongest "investigate here" signals available and should behave like one.
...
- **Hazards**: chain-reacting explosive barrels — damage + shove nearby entities, scorch the ground into the persistent decal layer, shake the camera.
- **Endless difficulty scaling** (`difficulty.js`): enemy count, health, damage and boss cadence all read off smooth curves keyed to stage number, not a handful of hardcoded per-stage tables that quietly stop scaling once someone stops adding new rows — an endless mode needs a formula, not a lookup table with a ceiling nobody remembered to raise.
...
- **The Trader**: a direct-purchase shelf, priced *relative to the crate cost* so a known rare costs roughly one crate-pull's worth of scrap and a legendary costs several — buying certainty should cost a premium over gambling for it, not be cheaper.
...
Four presets (`engine/quality.js`), auto-selected by device signal at first boot and always player-overridable, with the runtime allowed a small, strictly-monotonic, bounded budget of automatic step-downs if sustained frame time is poor even on a pinned choice (bounded so one bad minute early in a session can never permanently strand a device on the lowest tier for the rest of it):
...
- **Never let AdMob test IDs or a debug keystore reach a real store listing** — call this out explicitly in the release runbook, because it is exactly the kind of thing that is invisible in a working build and only surfaces as "why are we not earning ad revenue" weeks after shipping.
...
*Grounded against the live `public/game` source tree as of this document's writing (~19,500 lines of JS across `engine/`, `art/` and `game/`); every number, constant and data shape quoted above is copied directly from the shipped code, not reconstructed from memory.*
